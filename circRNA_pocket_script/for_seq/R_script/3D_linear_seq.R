  ...
  m_d_end<-0;
	for(j in 1:count_trans){
	       ...
	       for(k in 1:count_part){
                ...
                #mcol<-UTR_color
                ramp<- colorRamp(c(UTR_color_high, UTR_color))
                if(draw_type == "CDS"){
                   exon_b<<-1
                   ramp<- colorRamp(c(exon_color_high, exon_color))
                }
                if(draw_type == "intron"){
                   intron_b<<-1
                   ramp<- colorRamp(c(intron_color_high, intron_color))
                }
                ...
                colors <-rgb(ramp(seq(0, 1, length = 20)), max = 255)
                for(j in 1:20){
                   rect(d_sta, g_hight, d_end, g_hight-g_range*0.5/count_max_trans*m_w_vs_i*(1-j/20*0.7),col=colors[j],border=0)
                }
                rect(d_sta, g_hight, d_end, g_hight-g_range*0.5/count_max_trans*m_w_vs_i*0.3,col =colors[20],border=0,)
...
fun.seqdraw <- function(c,s,p,w){
   ...
   # pie(ss, col= mcol,radius = p,xlab = "",ylab = "",border=0,labels = "",clockwise=TRUE, init.angle = 90 )
    ...
    for(j in 1:50){
     rect((ss[1]+pos_sta-all_star)/all_len*100, seq_line_posh, (ss[1]+ss[2]+pos_sta-all_star)/all_len*100, seq_line_posh+seq_line_wid*(1-j/50),col=colors[j],border=0)
   }
}
...
mi_h=2;mi_m=1.2;sta_p=100;sta_l=90;m_l=2.8;m_shadow=0.15;m_cex=0.7
if (exon_b==1){
