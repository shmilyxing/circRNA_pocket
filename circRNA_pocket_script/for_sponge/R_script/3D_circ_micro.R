...
micro_color="darkgoldenrod2"
micro_color_high="darkgoldenrod3"
...
fun.mcolor <- function(col){
  if(col == "Blue") {
     # micro_color <<-"blue";micro_color_high <<-"#0000A7"
     micro_color <<-"#2E45F6";micro_color_high <<-"#0000A7"
   }
   if(col == "Red") {
     # micro_color <<-"#EE1A1A";micro_color_high <<-"#CD0F0F"
     micro_color <<-"#FF1313";micro_color_high <<-"#CD0F0F"
   }
   if(col == "Yellow") {
      #micro_color<<-"darkgoldenrod2";micro_color_high <<-"darkgoldenrod3"
      micro_color<<-"darkgoldenrod1";micro_color_high <<-"darkgoldenrod3"
   }
   if(col == "Green") {
     # micro_color<<-"green3";micro_color_high <<-"#00A400"
     micro_color<<-"#00DD00";micro_color_high <<-"#00A400"
   }
   if(col == "Purple") {
      #micro_color<<-"darkorchid3";micro_color_high <<-"darkorchid4"
      micro_color<<-"#B039EB";micro_color_high <<-"darkorchid4"
   }    
   if(col == "Brown") {
     # micro_color<<-"chocolate3";micro_color_high <<-"#A55217"
     micro_color<<-"#DF6E1F";micro_color_high <<-"#A55217"
   }
   if(col == "Grey") {
     # micro_color<<-"gray14";micro_color_high <<-"gray8"
     micro_color<<-"gray18";micro_color_high <<-"gray8"
   }
   if(col == "Pink") {
      # micro_color<<-"rosybrown3";micro_color_high <<-"#AF8484"
      micro_color<<-"#E5ADAD";micro_color_high <<-"#AF8484"
    }
    if(col == "Gold") {
      # micro_color<<-"brown2";micro_color_high <<-"goldenrod3"
      micro_color<<-"goldenrod1";micro_color_high <<-"goldenrod3"
    }
}
...
con_info <- file(paste(file_info, "/info.txt", sep=""), "r")
...
con_micro <- file(paste(file_info, "/info_micro.txt", sep=""), "r")
...
while(length(line) != 0 ) {
     ...
     if( b_micro_info ==0){
        v_trans<-c(v_trans,line)
        line<-readLines(con_micro,n=1)
     }
     else{
        s_micro_meg<<-paste(s_micro_meg, line,"\n")
        line<-readLines(con_micro,n=1)
     }
...
ramp_micro <- colorRamp(c(micro_color, micro_color_high)) 
...
nn=30
colors_micro<-rgb( ramp_micro(seq(0, 1, length = nn)), max = 255)
fun.draw_micro <- function(s,exon_r,exon_l){
      ...
      ss[3]=lapply(s[3],as.numeric)[[1]]
       ...
       #pie(ss, col = mcol, angle = 45,radius = exon_r,xlab = "",ylab = "",border=0,labels = "",density = NULL,lty = NULL,clockwise=my_clockwise, init.angle = if(my_clockwise) 90 else 0)

      # par(new=TRUE,fig=c(0,0.9,0.05,0.95))
      # pie(1, col = "white",radius = exon_l,xlab = "",ylab = "",border=0,labels = "")
...
for(j in 1:nn){
  ...
  par(new=TRUE,fig=c(0,0.9,0.05,0.95))
  pie(ss, col = b3,radius = (exon_l+(exon_r-exon_l)*(1-j/nn)),xlab = "",ylab = "",border=0, labels = "",clockwise=my_clockwise, init.angle = if(my_clockwise) 90 else 0)
}

     ss2=array(0,dim=c(3,1))
      ss2[1]=lapply(s[1],as.numeric)[[1]]
      ss2[2]=0.5
      ss2[3]=(ss[2]+ss[3]-1)
       mcol[2]="black"
       par(new=TRUE,fig=c(0,0.9,0.05,0.95),font.main=4)
       pie(ss2, col = mcol, angle = 45,radius = exon_r,xlab = "",ylab = "",border=0,labels = "",density = NULL,lty = NULL,clockwise=my_clockwise, init.angle = if(my_clockwise) 90 else 0)
      
}


fun.draw_micro_linear <- function(y,ty,gt,gb){

     l=3
    seql=0
    yy=array(0,dim=c(3,1))
    ...
    #rect(gene_line_posw+yy[1]*rate, gt, gene_line_posw+(yy[1]+yy[2])*rate, gb,col=micro_color,border=0)
   ...
   for(j in 1:l){
    ...
    rect(gene_line_posw+yy[1]*rate, gt, gene_line_posw+yy[1]*rate+gene_line_long*0.0005, gb,col="black",border=0)
}

for(p in 1:as.numeric(count_circ)){

      b_trans=0;
     for(p_trans in 1:length(v_trans)){
          temp<-strsplit(v_trans[p_trans],";")[[1]]
          if(temp[1] == ios_name[p] || temp[1] == "all"){
               b_trans<<-1;
               break;
          }
      }
...
browers <- read.table(paste(file_info, "/",ios_name[p], sep="") ,stringsAsFactors = FALSE)
...
colors_UTR<-rgb( ramp_UTR(seq(0, 1, length = nn)), max = 255)
    ...
    for(p_trans in 1:length(v_trans)){
          temp<-strsplit(v_trans[p_trans],";")[[1]]
          if(temp[1] == ios_name[p] || temp[1] == "all"){
             for(p_trans_micro in 3:(3+as.numeric(temp[2])-1)){           
                 fun.draw_micro(strsplit(temp[p_trans_micro],"\t")[[1]],(exon_r+be_link*0.2)*my_size,(exon_r-(exon_r-exon_l)*0.5)*my_size)
             }
            break;
          }
      }
      ...
      par(new=TRUE,fig=c(0,0.9,0.05,0.95),font.main=4)
      pie(y, col = ty_low,radius =(exon_r-(exon_r-exon_l)*0.5)*my_size,xlab = "",ylab = "",border=0, labels = "",clockwise=my_clockwise, init.angle = if(my_clockwise) 90 else 0)
      par(new=TRUE,fig=c(0,0.9,0.05,0.95))
      pie(y, col = ty,radius =(exon_r-(exon_r-exon_l)*0.395)*my_size,xlab = "",ylab = "",border=0, labels = "",clockwise=my_clockwise, init.angle = if(my_clockwise) 90 else 0)
 ...
 fun.geneline(y,b3,gene_line_posh,gene_line_posh+gene_line_wid*0.5*(1-j/nn))
     ...
     b4=array("",dim=c(3,0))
      for(p_trans in 1:length(v_trans)){
          ...
          if(temp[1] == ios_name[p] || temp[1] == "all"){
             for(p_trans_micro in 3:(3+as.numeric(temp[2])-1)){       
                # print(strsplit(temp[p_trans_micro],"\t")[[1]])    
                for(j in 1:nn){
                       
                    b4[2]=colors_micro[j]
                       #fun.geneline(y,b3,gene_line_posh,gene_line_posh+gene_line_wid*0.5*(1-j/nn))
                    fun.draw_micro_linear(strsplit(temp[p_trans_micro],"\t")[[1]],b4,gene_line_posh+gene_line_wid*0.5,gene_line_posh+gene_line_wid*0.5+gene_line_wid*0.7*(1-j/nn))
                 }
...
plot(1:100,1:100,type="n",xlab = "",ylab = "",axes = F)
#plot(1:100,1:100,type="n",xlab = "",ylab = "")
#text(0,40,s_micro_meg, cex = 0.7,pos=4)
text(50,50,s_micro_meg, cex = 0.7)