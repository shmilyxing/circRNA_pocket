...
fun.mcolor <- function(col){
  if(col == "Blue") {
   ...
   if(col == "Purple") {
      micro_color<<-"darkorchid3";
   }    
   if(col == "Brown") {
      micro_color<<-"chocolate3";
   }
   if(col == "Grey") {
      micro_color<<-"gray14";
   }
   if(col == "Pink") {
      micro_color<<-"rosybrown3";
    }
    if(col == "Gold") {
      micro_color<<-"brown2";
    }
...
fun.draw_micro <- function(s,exon_r){
...
fun.draw_micro_linear <- function(y,gt,gb){
    ...
    for(j in 1:l){
        temp<-(temp+yy[j])
        pos_x=gene_line_posw+pre*rate
        #rect(pos_x, gt, gene_line_posw+temp*rate, gb,col=mcol[j],border=0)
       #   x <- seq(pos_x,pos_x,length = 1000);
       #   y <- seq(gt,gb,length = 1000);
        # lines(x, y)
        #rect(pos_x, gt, pos_x+gene_line_long*0.0005, gb,col="black",border=0)
        if(j==1){
            #rect(gene_line_posw, gt, gene_line_posw+gene_line_long*0.0005, gb,col="black",border=0)
        }
        pre<-(pre+yy[j])
    }
     ...
     for(p_trans in 1:length(v_trans)){
          ...
          if(temp[1] == ios_name[p] || temp[1] == "all"){
             for(p_trans_micro in 3:(3+as.numeric(temp[2])-1)){           
                 fun.draw_micro(strsplit(temp[p_trans_micro],"\t")[[1]],(exon_r+be_link*0.2)*my_size)
             }
            break;
          }
      }
      par(new=TRUE,fig=c(0,0.9,0.05,0.95),font.main=4)
      pie(y, col = ty, angle = 45,radius = (exon_r+exon_l)*0.5*my_size,xlab = "",ylab = "",border=0,labels = "",density = NULL,lty = NULL,clockwise=my_clockwise, init.angle = if(my_clockwise) 90 else 0)
      par(new=TRUE,fig=c(0,0.9,0.05,0.95))
      pie(x, col = "white",radius = exon_l*my_size,xlab = "",ylab = "",border=0,labels = "")
          ...
          if(temp[1] == ios_name[p] || temp[1] == "all"){
             for(p_trans_micro in 3:(3+as.numeric(temp[2])-1)){       
                # print(strsplit(temp[p_trans_micro],"\t")[[1]])    
                 fun.draw_micro_linear(strsplit(temp[p_trans_micro],"\t")[[1]],gene_line_posh+gene_line_wid*0.5,gene_line_posh+gene_line_wid*1.2)
             }
             break;
          }
      }
